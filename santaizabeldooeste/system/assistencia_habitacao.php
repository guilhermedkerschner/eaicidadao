...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['acao']) && $_POST['acao'] == 'atualizar_status') {
        ...
        try {
            // Primeiro busca o número de protocolo, status atual e ID do usuário usando o ID da inscrição
            $stmt_protocolo = $conn->prepare("SELECT c.cad_social_protocolo, c.cad_social_status, c.cad_usu_id 
                                            FROM tb_cad_social c 
                                            WHERE c.cad_social_id = :id");
            $stmt_protocolo->bindParam(':id', $inscricao_id);
            $stmt_protocolo->execute();
            
            if ($stmt_protocolo->rowCount() > 0) {
                // Obtém o protocolo, status atual e ID do usuário
                $row = $stmt_protocolo->fetch(PDO::FETCH_ASSOC);
                $protocolo = $row['cad_social_protocolo'];
                $status_anterior = $row['cad_social_status'];
                $usuario_cidadao_id = $row['cad_usu_id']; // ID do usuário/cidadão que fez a solicitação
                
                // Atualiza o status na tabela tb_cad_social
                ...
                // Atualiza o status na tabela tb_solicitacoes
                $stmt_sol = $conn->prepare("UPDATE tb_solicitacoes SET status = :status WHERE protocolo = :protocolo");
                $stmt_sol->bindParam(':status', $novo_status);
                $stmt_sol->bindParam(':protocolo', $protocolo);
                $stmt_sol->execute();
                ...
                $stmt_sol_id->bindParam(':protocolo', $protocolo);
                $stmt_sol_id->execute();
                
                if ($stmt_sol_id->rowCount() > 0) {
                    $sol_row = $stmt_sol_id->fetch(PDO::FETCH_ASSOC);
                    $solicitacao_id = $sol_row['solicitacao_id'];
                    
                    // Registra a alteração no histórico de solicitações
                    // Nota: Usando o ID do usuário/cidadão para a chave estrangeira
                    $stmt_hist_sol = $conn->prepare("INSERT INTO tb_solicitacoes_historico 
                        (solicitacao_id, status_anterior, status_novo, detalhes, usuario_operacao, data_operacao) 
                        VALUES (:solicitacao_id, :status_anterior, :status_novo, :detalhes, :usuario, NOW())");
                    ...
                    $stmt_hist_sol->bindParam(':detalhes', $observacao);
                    $stmt_hist_sol->bindParam(':usuario', $usuario_cidadao_id); // Usando ID do usuário/cidadão!
                    $stmt_hist_sol->execute();
                }
                
                // Registra a alteração no histórico de cadastro social
                if (!empty($observacao)) {
                    ...
                    $acao = "Alteração de status para: " . $novo_status;
                ...
                $mensagem = "Status atualizado com sucesso e histórico registrado!";
